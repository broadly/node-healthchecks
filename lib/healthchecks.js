...
function get(url, headers, resolver, timeout, state) {
      ...
      .on('response', function(response) {
        ...
        if (response.statusCode < 200 || response.statusCode >= 400)
          ...
          if (state.redirects < 10) {
            const redirectURL = resolver(response.headers.location);
            if (withinSameDomain(redirectURL, loopbackURL))
              get(response.headers.location, headers, resolver, timeout, { redirects: state.redirects + 1 }).then(resolve, reject);
            else
              resolve({ statusCode: response.statusCode, body: null, elapsed: elapsed });
          }
          else
            reject({ error: new Error('too many redirects'), elapsed: elapsed });
        }
        else {
          const buffers = [];
