...
function runCheck(url, headers, resolver, timeout) {
  return new Promise(function(resolve, reject) {
    const queue     = new EventEmitter;
    const redirects = 0;

    queue.on('request', function(url, redirects) {
          ...
          if (res.statusCode > 300 && res.statusCode < 400) {
            if (redirects < 10) {
              ...
              if (withinSameDomain(redirectURL, loopbackURL))
                queue.emit('request', res.headers.location, redirects + 1);
              else
                resolve(res);
            } else
              reject({ error: new Error('too many redirects'), elapsed: res.elapsed });
          } else
            resolve(res);
        })
        .catch(function(err) {
...
//
// On success, promise is resolved with { statusCode, headers, body, elapsed }.
// On error or timeout, promise is rejected with { error, elapsed }.
//
