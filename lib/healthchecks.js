...
function checkFunction(url, requestID) {
  ...
  const allChecks = Object.keys(checks).map(function(checkURL) {
    ...
    return new Promise(function(resolve) {
      ...
      runCheck({ url: checkURL, headers, resolver: loopbackResolve, timeout })
        .then(function(res) {
          const elapsed = process.hrtime(start);
          const outcome = new Outcome({ url: checkURL, expected, statusCode: res.statusCode, body: res.body, elapsed });
          resolve(outcome);
        })
